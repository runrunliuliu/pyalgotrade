# -*- coding: utf-8 -*-
"""
Created on Mon Dec 21 16:14:37 2015
...
class Series(object):
    ...
    # use numpy array None ==> np.nan
    def datevalues(self,dateTimes):
        lenth  = len(dateTimes)
        values = np.empty(lenth)
        index  = 0
        for dateTime in dateTimes:
            # values.append(self.getValue(dateTime))
            # times.append(dateTime)
            val = self.getValue(dateTime)
            values[index] = np.nan if val is None else val
            index = index + 1
...
class Subplot(object):
    ...
    def __init__(self):
        ...
        self.__fig = figure(x_axis_type="datetime",width=1000,plot_height=500,title=None)
...
class InstrumentSubplot(Subplot):
    ...
    def onOrderEvent(self, broker_, orderEvent):
        ...
        if self.__plotBuySell and  \
                orderEvent.getEventType() in (broker.OrderEvent.Type.PARTIALLY_FILLED, broker.OrderEvent.Type.FILLED) and \
...
class StrategyPlotter(object):
    ...
    def __buildFigureImpl(self, fromDateTime=None, toDateTime=None):
        ...
        for i, subplot in enumerate(subplots):
            fig  = subplot.getFig()
            series = subplot.getseries()
            numlines  = len(series)
            mypalette = Spectral11[0:numlines]
            index = 0
            for name,sers in series.iteritems():
                times,values = sers.datevalues(dateTimes)
                if name == 'Buy':
                    fig.triangle(times,values,size=10,color="#DE2D26",legend=name)
                if name == 'Sell':
                    fig.inverted_triangle(times,values,size=10,color="#99D594",legend=name)
                fig.line(times,values,line_color=mypalette[index],line_width = 2,legend=name)
                index = index + 1 

            self.vplots.append(fig)
        ...
        if self.__portfolioSubplot is not None:
            fig = self.__portfolioSubplot.getFig() 
            series = self.__portfolioSubplot.getseries()
            for name,sers in series.iteritems():
                times,values = sers.datevalues(dateTimes)
                fig.line(times,values,line_width = 2,legend=name)
            self.vplots.append(fig)
    ...
    def plot(self, fromDateTime=None, toDateTime=None):
        ...
        :type toDateTime: datetime.datetime
        """
        self.__buildFigureImpl(fromDateTime, toDateTime)
